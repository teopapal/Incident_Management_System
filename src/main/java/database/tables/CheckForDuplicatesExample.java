...
public class CheckForDuplicatesExample {
    ...
    public boolean isPhoneAvailable(String phone) throws SQLException, ClassNotFoundException {
        Connection con = DB_Connection.getConnection();
        Statement stmt = con.createStatement();

        ResultSet rs;
        try {
            rs = stmt.executeQuery("SELECT COUNT(telephone) AS total FROM users WHERE telephone = '" + phone + "'");
            rs.next();
            if (rs.getInt("total") == 0) {
                rs = stmt.executeQuery("SELECT COUNT(telephone) AS total2 FROM volunteers WHERE telephone = '" + phone + "'");
                rs.next();
                if (rs.getInt("total2") == 0) {
                    return true;
                }
